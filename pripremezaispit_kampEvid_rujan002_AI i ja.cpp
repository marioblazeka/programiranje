...
bool nePostoji = false; // Novi kod – poèetno pretpostavljamo da kamp mjesto postoji tek kad ga unesemo
...
void unosNovZap() {
    ...
    // Provjera da li ID veæ postoji
    for (int i = 0; i < unos; i++) {
        if (evidencija[i].IDbroj == evidencija[unos].IDbroj) { // STARI KOD: if(unos==evidencija[unos].IDbroj) › greška, usporeðivao unos s ID
