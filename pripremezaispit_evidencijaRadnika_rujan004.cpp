...
void pretragaPoGodRod(){
	...
	do{		
	cout<<"-----IZBORNIK-----\n";
	cout<<"1. Upisi godinu rodjenja za pretragu \n";
	cout<<"2. Upisi mjesec rodjenja za pretragu\n";
	cin>> pretraga;
	
	switch(pretraga){
		case 1:	pretragaPoGodRod(); break;
		case 2:	pretragaPoGodRod(); break;
...
cout<<"Upisani radnici su prosjecno stari "<< prosjecnaStar/brojUnosa<<endl;
cout<<"Prosjek place upisanih radnika je "<<prosPlac/brojUnosa<<endl;
}
}
}
}
...
void spremanje(){
	...
	ofstream datoteka ("podaci.txt");
		...
		datoteka<< " ime i prezime \t radna knjizica \t godina rodjenja\n";
	for(int i=0; i<brojUnosa;i++){
	datoteka<<evidencija[i].imePrez<<"\t"<<evidencija[i].brojRk<<"\t"<<evidencija[i].datumRod<<endl;
...
void izbornik(){
	...
	cout<<"5. Spremanje radnika u podaci.txt \n";
