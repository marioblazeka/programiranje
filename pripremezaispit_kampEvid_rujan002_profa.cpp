...
void unos(Kamp *polje, int &l) {
  ...
  cin >> id_kampa;
  // provjera da li mjesto veæ postoji u polju
  if (postoji(polje, l, id_kampa)) {
    cout << "Kamp mjesto je veæ rezervirano" << endl;
  ...
  // premjesti brojac na sljedeæe prazno mjesto za sljedeæi upis
  l++;
}
...
void izmjena(Kamp *polje, int l) {
  ...
  for (int i = 0; i < l; i++) {
    if (polje[i].id == id_kampa) {
      ...
      cout << "2. Razbacivanje smeæa: 400 kn" << endl;
...
void naplata(Kamp *polje, int &l) {
  ...
  for (int i = 0; i < l; i++) {
    if (polje[i].id == id_kampa) {
      ...
      polje[i].prekrsaji = 0;
      polje[i] = polje[l - 1];
      // sveukupno je sada jedan manje element, pa smanji brojaè
      l--;
